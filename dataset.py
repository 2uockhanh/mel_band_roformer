...
from valid import find_mixture_files

warnings.filterwarnings("ignore")
# Tải các phân đoạn của bài hát
def load_chunk(path, length, chunk_size, offset = None, target_channels = 2):
    ...
    return x.T # Returns array with shape (target_channels, chunk_size)
# Lấy đường dẫn và độ dài của mỗi track
def get_track_set_length(params):
    path, instruments, file_types, dataset_type = params
    should_print = (not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0) and dataset_type != 7
    # Kiểm tra dộ dài của các loại stem nhạc cụ (số lượng khác nhau ở một số trường hợp)
    lengths_arr = []
    for instr in instruments:
        ...
        if length == -1:
            if should_print:
                print(f'Can\'t find file "{instr}" in folder {path}')
            continue
    ...
    # Ta dùng độ dài nhỏ nhất để cho phép overflow cho soundfile đọc trong trường hợp độ dài không bằng nhau
    return path, lengths_arr.min()
# Chỉ dùng khi truyền tham số --dataset_type = 4 hoặc 5 hoặc 6
def process_chunk_worker(args):
    ...
    try:
        for instrument in instruments:
            ...
            for extension in file_types:
                path_to_audio_file = track_path + f'/{instrument}_{extension}'
                if os.path.isfile(path_to_audio_file):
                    try:
        ...
        return (track_path, offset, False)
# Chuẩn bị tập dữ liệu huấn luyện (Hiện tại hàm này không dùng)
...
def get_track_length(params):
    ...
    return (path, length)
# Tập dữ liệu Validation, bao gồm tất cả các file mixture.wav của tất cả các bài hát trong tập dữ liệu
    ...
    def __init__(self, args):
        ...
        for path in args.valid_path:
            ...
            for child in child_paths:
                ...
                if not part:
                    print(f'No validation data found in: {os.path.join(args.valid_path, child)}')
...
class MSSDataset(torch.utils.data.Dataset):
    def __init__(self, config, data_path, batch_size = None, metadata_path = "metadata.pkl", dataset_type = 1, verbose = True):
    ...
    def __getitem__(self, index):
        ...
        if self.aug:
            if 'mp3_compression_on_mixture' in self.config['augmentations']:
                apply_aug = audiomentations.Mp3Compression(
        ...
        return res, mix
    # Chỉ dùng khi truyền giá trị biến --dataset_type = 7
    def _build_class_to_tracks(self):
        ...
        if os.path.isfile(cache_path):
            ...
            if (cache.get('total_tracks') == total_tracks and cache.get('max_ratio') == max_ratio):
                ...
                if should_print:
                    print(f'[dataset_type = 7] Loaded {len(self.available_classes)} classes from cache')
                return
            else:
                if should_print:
        ...
        for instr, tracks in self.class_to_tracks.items():
            count = len(tracks)
            ...
            if ratio > max_ratio:
                if should_print:
                    print(f"[dataset_type = 7] Skip frequent stem '{instr}': {count}/{total_tracks} ({ratio:.1%})")
                continue
            filtered_class_to_tracks[instr] = tracks
        if len(filtered_class_to_tracks) == 0:
            raise RuntimeError('dataset_type 7: all classes were filtered out by frequency threshold')
        ...
        with open(cache_path, 'w', encoding = 'utf8') as f:
            json.dump({'total_tracks': total_tracks, 'max_ratio': max_ratio, 'class_to_tracks': filtered_class_to_tracks, }, f, indent = 2)
        if should_print:
            print(f"[dataset_type = 7] Using {len(self.available_classes)} balanced classes out of {len(self.instruments)} instruments")
    # Chỉ dùng khi truyền giá trị biến --dataset_type = 7
    ...
    # Chỉ dùng khi truyền giá trị biến --dataset_type = 4 hoặc 5 hoặc 6
    def _initialize_chunks_metadata(self):
        should_print = (not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0)
        chunks_cache_path = self.metadata_path.replace('.pkl', '_chunks.pkl')
        current_config = {
            'chunk_size': self.chunk_size, 'min_means_abs': self.min_mean_abs, 'instruments': sorted(self.instruments)
        }
        if os.path.exists(chunks_cache_path):
            try:
                ...
                config_matches = (
                    ...
                    cached_config.get('min_mean_abs') == current_config['min_means_abs'] and
    ...
    # Chỉ dùng khi truyền tham số --dataset_type = 4 hoặc 6
    def _precompute_chunks(self):
        ...
        if torch.multiprocessing.cpu_count() > 1:
        ...
        if self.verbose and should_print:
            print(f'Created {len(chunks_metadata)} good chunks from {len(self.metadata)} tracks')
        return chunks_metadata
    # Chỉ dùng khi truyền tham số --dataset_type = 5
    def _precompute_random_chunks(self):
        ...
        with tqdm(total = target_count, desc = 'Progress good chunks') as pbar:
            while len(chunks_metadata) < target_count:
                ...
                print(f'\nTarget count: {target_count}. Chunks Metadata length: {len(chunks_metadata)}')
            print(f'Good chunks length: {len(good_chunks)}. Need: {need}')
    ...
    def _is_chunk_loud_enough(self, track_path, offset, chunk_size, track_length):
        try:
            for instrument in self.instruments:
                instrument_loud_enough = False
                for extension in self.file_types:
                    path_to_audio_file = track_path + f'/{instrument}.{extension}'
                    if os.path.isfile(path_to_audio_file):
                        try:
                            source = load_chunk(path_to_audio_file, length = track_length, offset = offset, chunk_size = chunk_size)
                            ...
                            if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
                                print(f'Error loading: {e} Path: {path_to_audio_file}')
                            return False
                if not instrument_loud_enough:
                    return False
            return True
            ...
            if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
                print(f'Error checking chunk loudness: {e} Path: {track_path}')
            return False
    # Chỉ dùng khi truyền giá trị biến --dataset_type = 5 hoặc thực hiện phân đoạn âm thanh
    ...
    # Chỉ dùng khi truyền giá trị biến --dataset_type = 6
    def load_aligned_data(self):
        ...
        while attempts:
            ...
            mix = None
            for extension in self.file_types:
                path_to_mix_file = track_path + f'/mixture.{extension}'
                if os.path.isfile(path_to_mix_file):
                    try:
                        mix = load_chunk(path_to_mix_file, track_length, self.chunk_size, offset = common_offset)
                    except Exception as e:
                        if should_print:
                            print(f'Error loading mix: {e} Path: {path_to_mix_file}')
                    break
            if silent_chunks == 0:
                break
            attempts -= 1
        ...
        try:
            ...
            print(f'Error during stacking stems: {str(e)} Track Length: {track_length} Track path: {track_path}')
    ...
    # Nếu như file metadata có sẵn, đọc để lấy thông tin
    def read_from_metadata_cache(self, track_paths, instr = None):
        ...
        # Ta không đọc lại các track cũ trong file metadata cũ    
        ...
        return track_paths, metadata
    # Lấy metadata từ dataset và lưu vào trong file pickle (.pkl)
    def get_metadata(self):
        read_metadata_procs = torch.multiprocessing.cpu_count() - 2
        should_print = not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0

        if self.verbose and should_print:
            print(
                f'Dataset type: {self.dataset_type}',
                f'Processes to use: {read_metadata_procs}',
        ...
        if self.dataset_type in [1, 4, 5, 6, 7]:
            ...
            if type(self.data_path) == list:
                for tp in self.data_path:
                    if 'train' in os.listdir(tp):
                        tracks_for_folder = sorted(glob(tp + '/train/*'))
                    if len(tracks_for_folder) == 0 and should_print:
                        print(f'Warning: no tracks found in folder \'{tp}\'train. Please check it!')
            ...
            if read_metadata_procs <= 1:
                ...
                with ThreadPoolExecutor(max_workers = read_metadata_procs) as executor:
                    futures = [
                        ...
                        for args in zip(track_paths, itertools.repeat(self.instruments), itertools.repeat(self.file_types), itertools.repeat(self.dataset_type), )
                    ]
                    if should_print:
            ...
            if should_print:
                print(f'Unknown dataset type: {self.dataset_type}. Must be 1, 4, 5, 6, 7.')
            exit()
        # Lưu dữ liệu các bài hát huấn luyện trong metadata
        pickle.dump(metadata, open(self.metadata_path, 'wb'))
        return metadata
    # Tải file âm thanh thành phần (vocals, other,...) từ file metadata.pkl
    def load_source(self, metadata, instr):
        should_print = (not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0)
        while True:
            if self.dataset_type in [1, 4, 5, 6, 7]:
                for i in range(len(metadata)):
                    track_path = metadata[i][0]
                    track_length = metadata[i][1]
                    found_any = False
                    for extension in self.file_types:
                        path_to_audio_file = track_path + f'/{instr}.{extension}'
                        if os.path.isfile(path_to_audio_file):
                            ...
                            try:
                                ...
                                # Đôi khi lỗi này sẽ xuất hiện khi đọc file định dạng FLAC, bắt nó và dùng zero stem
                                if should_print:
                                    print(f'Error: {e} Path: {path_to_audio_file}')
                                source = np.zeros((2, self.chunk_size), dtype = np.float32)
                            break
                    if not found_any:
                        raise FileNotFoundError(f"Require stem '{instr}' not found in track folder '{track_path}' with extensions {self.file_types} .")
            else:
    ...
    # Tải ngẫu nhiên các file âm thanh hỗn hợp
    def load_random_mix(self):
        res = []
        for instr in self.instruments:
            s1 = self.load_source(self.metadata, instr)
            # Nhiều hỗn hợp các loại stems giống nhau
            if self.aug:
        ...
        res = torch.stack(res)
        return res
    # Dữ liệu mở rộng như xào kênh (channel_shuffle), đảo ngược ngẫu nhiên (random_inverse), phân cực ngẫu nhiên (random_polarity)
    def augm_data(self, source, instr):
        # source.shape = (first_channels, second_channels)
        ...
        if 'all' in self.config['augmentations']:
            ...
            augs = dict()
        # Ta cân thêm tất cả các augs cho stem. Và ghi lại giá trị nếu cần thiết
